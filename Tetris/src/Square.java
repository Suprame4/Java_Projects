...
public class Square {
	...
	public boolean canMove(Direction direction) {
		...
		switch (direction) {
		case DOWN:
			//the first condition checks if you're currently on the bottom row 
			//the second condition checks if there's a square one row below where you're currently at
		...
		case LEFT:
			//create the conditionals for left movement 
			if (col == 0 || grid.isSet(row, col - 1))
				move = false;
			break;
		case RIGHT:
			//create the conditionals for right movement 
	...
	public void move(Direction direction) {
		if (canMove(direction)) {
			...
			//left movement 
			case LEFT:
				//move to the left of the 2D array, meaning column - 1
				col--;
				break;
			
			//right movement 
			case RIGHT:
				//move to the right of the 2D array, meaning column + 1
				col++;
				break;
